...
// Only the 13 missing low suit cards are reconstructed, using this deck's
// own four suit marks on the requested uniform white background.
...
try {
  ...
  symbols = await page.evaluate(async ({ src }) => {
    ...
    return result;
  }, { src: `data:image/png;base64,${atlas.toString('base64')}` });
} finally {
  await browser.close();
}
...
`;
  ...
  generated.push({
    ...
    atlasBounds: symbol.bounds, background: '#ffffff',
    treatment: 'AI-assisted extraction of an original Smrekar suit mark; self-contained SVG with exact pip count and a uniform white background. Reconstructed card, not an original scan.',
